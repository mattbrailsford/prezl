...
Each stage (or step) can declare one or more **demos** — URLs and
fullscreen videos that the **Run** button launches. Screens with no
demo keep the Run button disabled; screens with one or more light it
up.

## The Run flow

1. Click **Run** (or press <kbd>F5</kbd> / <kbd>Ctrl</kbd>+<kbd>F5</kbd> / <kbd>Ctrl</kbd>+<kbd>Enter</kbd>).
2. If the screen has more than one demo, a **picker** opens —
   arrow keys cycle, Enter selects, Esc closes.
3. Status bar flashes `Preparing…` → `Ready` → `Launching demo…` →
   `Ready`. ~900 ms of dramatic pause to give the action a beat.
4. The chosen demo fires:
   - **URL demo** — hands the URL to the OS default browser and
     returns to idle.
   - **Video demo** — opens a fullscreen modal with the clip.

Only one demo is active at a time; the picker (and the autoLaunch
logic) won't stomp a modal that's already up.

## YAML shape — `demo:` vs `demos:`

Two field names, both accepted; using both on the same stage/step is a
parse error.

```yaml
# Singular shorthand — one demo, write the object directly.
demo:
  type: url
  src: https://example.com/dashboard

# Plural — explicit list of one or more demos.
demos:
  - type: video
...
better — `demo:` for the common single-demo case, `demos:`
...
- **`cues`** — timestamps (seconds) where playback auto-pauses. Each
  ...
  narration. A bare number is shorthand for `{ time: <number> }`; the
  object form lets you attach an optional `title:`.

## Picker labels — `title:`

Each demo entry accepts an optional `title:` string. The picker uses
...
A `demo:` (or `demos:`) declared on a step replaces the stage's
...
- A step that omits `demo:` / `demos:` uses the **stage default**.
- A step that declares its own value uses that value (replaces the list
  wholesale; we don't merge).
- `demo: ~` (YAML null) means *explicitly empty for this step* —
  this screen has no demos even though the stage does.

So in the example above, `shell.intro` and `shell.showApi` both inherit
the stage's URL demo by stage→step inheritance; `shell.liveDemo`
swaps it for the video. Adding a fourth step that omits the field would
land back on the stage's URL demo — there's no carry-forward of
`liveDemo`'s override.
